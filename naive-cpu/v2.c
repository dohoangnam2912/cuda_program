...
void forward(NeuralNetwork *nn, float *input, float *hidden1, float *hidden2, float *output, int batch_size) {
    // Input to Hidden 1
    matmul_a_b(input, nn->weights1, hidden1, batch_size, INPUT_SIZE, HIDDEN_SIZE);
    
    // Add bias1
    bias_forward(hidden1, nn->bias1, batch_size, HIDDEN_SIZE);
    
    // Apply ReLU
    relu_forward(hidden1, batch_size * HIDDEN_SIZE);

    // Hidden 1 to Hidden 2
    matmul_a_b(hidden1, nn->weights2, hidden2, batch_size, HIDDEN_SIZE, HIDDEN_SIZE);
    
    // Add bias1
    bias_forward(hidden2, nn->bias2, batch_size, HIDDEN_SIZE);
    
    // Apply ReLU
    relu_forward(hidden2, batch_size * HIDDEN_SIZE);

    // Hidden 2 to Output
    matmul_a_b(hidden2, nn->weights3, output, batch_size, HIDDEN_SIZE, OUTPUT_SIZE);

    // Add bias3
    bias_forward(output, nn->bias3, batch_size, OUTPUT_SIZE);
...
void backward(NeuralNetwork *nn, float *input, float *hidden1, float *hidden2, float *output, int *labels, int batch_size) {
    
    // Initialize gradients to zero
    zero_grad(nn->grad_weights1, HIDDEN_SIZE * INPUT_SIZE);
    zero_grad(nn->grad_weights2, HIDDEN_SIZE * HIDDEN_SIZE);
    zero_grad(nn->grad_weights3, OUTPUT_SIZE * HIDDEN_SIZE);
    zero_grad(nn->grad_bias1, HIDDEN_SIZE);
    zero_grad(nn->grad_bias2, HIDDEN_SIZE);
    zero_grad(nn->grad_bias3, OUTPUT_SIZE);
    ...
    // Update gradients for weights3 (W3.grad = grad_output.T @ hidden2)
    matmul_at_b(hidden2, grad_output, nn->grad_weights3, batch_size, HIDDEN_SIZE, OUTPUT_SIZE);

    // Update gradients for bias3
    bias_backward(nn->grad_bias3, grad_output, batch_size, OUTPUT_SIZE);

    // Compute dX3 (gradient of loss w.r.t. input of second layer)
    float *dX3 = malloc(batch_size * HIDDEN_SIZE * sizeof(float));

    // grad_output @ W3.T = dX3 -> (B, 1024) @ (1024, 256) = (B, 256)
    matmul_a_bt(grad_output, nn->weights3, dX3, batch_size, OUTPUT_SIZE, HIDDEN_SIZE);

    // Compute d_ReLU_out (element-wise multiplication with ReLU derivative)
    float *d_ReLU_out2 = malloc(batch_size * HIDDEN_SIZE * sizeof(float));
    for (int i = 0; i < batch_size * HIDDEN_SIZE; i++) {
        d_ReLU_out2[i] = dX3[i] * (hidden2[i] > 0);
    }
    // retains its shape since its just a point-wise operation
    // Update gradients for weights1 (W1.grad = d_ReLU_out.T @ input)
    matmul_at_b(hidden1, d_ReLU_out2, nn->grad_weights2, batch_size, HIDDEN_SIZE, HIDDEN_SIZE);

    // Update gradients for bias1
    bias_backward(nn->grad_bias2, d_ReLU_out2, batch_size, HIDDEN_SIZE);
    // ---------------------------------------------------------
    // Compute dX2 (gradient of loss w.r.t. input of second layer)
    float *dX2 = malloc(batch_size * HIDDEN_SIZE * sizeof(float));

    // grad_output @ W3.T = dX3 -> (B, 1024) @ (1024, 256) = (B, 256)
    matmul_a_bt(nn->grad_weights3, nn->weights2, dX2, batch_size, HIDDEN_SIZE, HIDDEN_SIZE);

    // Compute d_ReLU_out (element-wise multiplication with ReLU derivative)
    float *d_ReLU_out1 = malloc(batch_size * HIDDEN_SIZE * sizeof(float));
    for (int i = 0; i < batch_size * HIDDEN_SIZE; i++) {
        d_ReLU_out1[i] = dX2[i] * (hidden1[i] > 0);
    }
    // retains its shape since its just a point-wise operation
    // Update gradients for weights1 (W1.grad = d_ReLU_out.T @ input)
...
void train(NeuralNetwork *nn, float *X_train, int *y_train) {
    float *hidden1 = malloc(BATCH_SIZE * HIDDEN_SIZE * sizeof(float));
    float *hidden2 = malloc(BATCH_SIZE * HIDDEN_SIZE * sizeof(float));
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        for (int batch = 0; batch < num_batches; batch++) {
            ...
            forward(nn, &X_train[start_idx * INPUT_SIZE], hidden1, hidden2, output, BATCH_SIZE);
            ...
            backward(nn, &X_train[start_idx * INPUT_SIZE], hidden1, hidden2, output, &y_train[start_idx], BATCH_SIZE);
            update_weights(nn);
